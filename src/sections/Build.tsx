...
const PHASES: Phase[] = [
  ...
  {
    // Phase 3 is fully gold to match phases 1 and 2's color cohesion: gold
    // pill, gold bullet markers, gold breathing dot, gold line above.
...
// Each phase entry renders its own line segment that runs ABOVE its dot —
// for phase 1 it extends up into the timeline's pt padding so the cyan
// segment is visible; for phase 2/3 it uses a negative top to span the
// `space-y` gap that the parent <ol> creates between siblings, plus the
// 8px offset down to the dot's vertical center.
function PhaseEntry({ phase, isLit, isFirst, phaseRef }: PhaseEntryProps) {
  return (
    <li ref={phaseRef} className="relative pl-8 sm:pl-10">
      <span
        ...
        className={`absolute left-[7px] w-px transition-all duration-[600ms] ease-out ${
          isFirst
            ? "-top-8 sm:-top-10 h-[calc(2rem+8px)] sm:h-[calc(2.5rem+8px)]"
            : "-top-12 sm:-top-14 h-[calc(3rem+8px)] sm:h-[calc(3.5rem+8px)]"
        }`}
        style={{
          background: isLit ? phase.litColor : "rgb(148,163,184)",
...
export function Build() {
  ...
  // Each phase lights independently as soon as it crosses the 50% viewport
  // threshold. Lit state is monotonic — once lit, scrolling back up doesn't
  // dim it. This makes the timeline progressive (each segment fires when
  // the user scrolls to it) rather than all-at-once.
  ...
  return (
    ...
    >
      <div className="max-w-[720px] mx-auto w-full">
        ...
        {/* pt-8 / pt-10 gives the phase-1 cyan segment somewhere visible to
            extend above its dot. Without this padding the segment would be
            zero-length and effectively invisible. */}
